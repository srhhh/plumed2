...
void Mapping::mergeDerivatives( const unsigned& ider, const double& df ){
  unsigned frameno=ider*low_dim.size() + current;
  for(unsigned i=0;i<getNumberOfArguments();++i){
      accumulateDerivative( i, df*dfframes[frameno]*frames[current]->getArgumentDerivative(i) );
  }
  if( getNumberOfAtoms()>0 ){
      Vector ader; Tensor tmpvir; tmpvir.zero();
      unsigned n=getNumberOfArguments(); 
      for(unsigned i=0;i<getNumberOfAtoms();++i){
          ader=frames[current]->getAtomDerivative(i);
      ...
      Tensor vir; 
      if( !frames[current]->getVirial( vir ) ) vir=tmpvir;
