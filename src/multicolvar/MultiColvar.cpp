...
void MultiColvar::readTwoGroups( const std::string& key1, const std::string& key2 ){
  ...
  for(unsigned i=0;i<t1.size();++i){
     for(unsigned j=0;j<t2.size();++j) addTaskToList( i*nblock + j );
  }
  if( !verbose_output ){
  ...
  if( t3.empty() && !allow2 ){
      ...
      for(unsigned i=0;i<t1.size();++i){
        for(unsigned j=1;j<t2.size();++j){ 
           for(unsigned k=0;k<j;++k) addTaskToList( nblock*nblock*i + nblock*j + k );
        }
      }
      if( !verbose_output ){
        log.printf("  constructing colvars from two groups containing %d and %d atoms respectively\n",t1.size(),t2.size() ); 
      ...
      for(unsigned i=0;i<t1.size();++i){
          for(unsigned j=0;j<t2.size();++j){
              for(unsigned k=0;k<t3.size();++k) addTaskToList( nblock*nblock*i + nblock*j + k );
          }
      }
      if( !verbose_output ){
