...
{\tt
\# the two path variables are the two components (sss,zzz) of the PATHMSD cv
\#\\
p1: PATHMSD REFERENCE=frames.pdb LAMBDA=10300
\#\\
...
\#\\
mtd: METAD ARG=p1.sss,p1.zzz PACE=200 HEIGHT=0.4 SIGMA=0.3,0.002 \\
\#\\
PRINT ARG=p1.sss,p1.zzz,phi,psi,mtd.bias STRIDE=100 FILE=COLVAR \\
\#\\
ENDPLUMED \\
}}
...
\#\\
UPPPER\_WALLS ARG=p1.zzz AT=0.0 KAPPA=100.0 \\
\#\\
\# a rather rude metadynamics. do not take this as production conditions\\
\# metadynamics only on S, the other is just for monitoring\\
\#\\
mtd: METAD ARG=p1.sss PACE=400 HEIGHT=0.4 SIGMA=0.25 \\
\#\\
PRINT ARG=p1.sss,p1.zzz,phi,psi,mtd.bias STRIDE=100 FILE=COLVAR \\
\#\\
ENDPLUMED \\
...
structural features:
the H-bonds (CVs {\tt COORD} ), the typical dihedrals (CVs {\tt TORSION},  {\tt ALPHABETA}), or the detailed
...
ALPHABETA ...
ATOMS1=7,9,21,23 REFERENCE=-0.78540	\\
ATOMS2=23,25,37,39\\
ATOMS3=39,41,53,55\\
ATOMS4=55,57,69,71\\
ATOMS5=71,73,85,87\\
ATOMS6=87,89,101,103\\
ATOMS7=103,105,117,119\\
ATOMS8=119,121,133,135\\
ATOMS9=135,137,149,151\\
ATOMS10=151,153,165,167\\
ATOMS11=167,169,181,183\\
ATOMS12=183,185,197,199\\
ATOMS13=199,201,213,215\\
ATOMS14=215,217,229,231\\
ATOMS15=231,233,245,247\\
ATOMS16=247,249,261,263\\
ATOMS17=263,265,277,279\\
ATOMS18=279,281,293,295\\
ATOMS19=295,297,309,311\\
LABEL=ab
... ALPHABETA\\

\# 5 kJ/mol (gromacs units) every 5 ps (timestep = 2 fs)\\
mtd: METAD ARG=ab HEIGHT=5.0 PACE=2500 SIGMA=1.0\\
PRINT ARG=ab,mtd.bias STRIDE=500 FILE=COLVAR\\
...
\esempio{ Input for secondary structure exploration in a Val$_{20}$ peptide\\ \\
{\tt
MOLINFO STRUCTURE=val20.pdb\\
alpha: ALPHARMSD RESIDUES=all TYPE=DRMSD LESS\_THAN={RATIONAL R\_0=0.08 NN=8 MM=12}\\
abeta: ANTIBETARMSD RESIDUES=all TYPE=DRMSD LESS\_THAN={RATIONAL R\_0=0.08 NN=8 MM=12} STRANDS\_CUTOFF=1.\\

\# 5 kJ/mol (gromacs units) every 5 ps (timestep = 2 fs)\\
mtd: METAD ARG=alpha.less\_then,beta.less\_then SIGMA=0.3,0.3 HEIGHT=5.0 PACE=2500\\
PRINT ARG=alpha.less\_then,beta.less\_then STRIDE=500 FILE=COLVAR\\ 
...
In order to activate the grid in \plumed, the keyword \tt{GRID} can be specified in \keyword{METAD} for every
...
{\tt
phi: TORSION ATOMS=5,7,9,15\\
mtd: METAD ARG=phi PACE=1000 HEIGHT=0.4 SIGMA=0.35 GRID\_MIN=-pi GRID\_MAX=pi GRID\_BIN=100 TEMP=300 BIASFACTOR=8\\
ENDPLUMED
}}
